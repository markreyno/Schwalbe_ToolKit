...
export function ToolCard({ tool }: { tool: Tool }) {
  ...
  return <article className={`tool-card ${tool.available ? 'available' : 'upcoming'}`}>
    <div className="card-top"><div className="tool-icon"><Icon size={30} strokeWidth={1.7}/></div><span className={`badge ${tool.available ? 'green' : ''}`}>{tool.available ? 'Available' : 'Coming later'}</span></div>
