...
export function ToolHub() {
  ...
  return <>
    <header className="page-heading"><div><p className="eyebrow">YOUR WORKDAY, SIMPLIFIED</p><h1>A place for every tool.</h1><p className="subtitle">Small tools. Less friction. More room to get things done.</p></div><button type="button" className="water-test-notification" onClick={() => setNotificationDemo(value => value + 1)}><Bell size={17}/> Test notification</button></header>
    {notificationDemo > 0 && <div key={notificationDemo} className="water-alert water-demo-alert" role="alert" aria-live="assertive" aria-atomic="true"><Forklift size={20}/><div><span className="water-demo-tag">DEMO NOTIFICATION</span><strong>1 picker needs water</strong><p>Demo Picker &middot; Watering overdue</p><p>This is a test announcement. Your picker schedules have not changed.</p></div><button type="button" aria-label="Dismiss test notification" onClick={() => setNotificationDemo(0)}><X size={18}/></button></div>}
